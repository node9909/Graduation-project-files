...
void setup(){
  ...
  //attachInterrupt(digitalPinToInterrupt(IPIN_DRDY), read_data_frame, FALLING);
  delay(500);
  //readid();
  delay(5000);
  delay(5000);
...
void ADS1298_INIT()
{
  //WiredSerial.println("Start Init");
  delay(500); //wait for the ads129n to be ready - it can take a while to charge caps
  ...
  delay(500);
  adc_wreg(CH3SET,0x91); // CH2 power down, gian 1, input shorted
  delay(500);
  ...
  attachInterrupt(digitalPinToInterrupt(IPIN_DRDY), read_data_frame, FALLING);
  //WiredSerial.println("Interrupt enabled");
  //WiredSerial.println("Init Done");
}
...
void read_data_frame()
{
  ...
  for (int i = 0; i < 9; i++) {
    ...
    val2 = data[1+(3*i)];
    val3 = data[2*(3*i)];
    data1[i] = (val1 << 16) | (val2 << 8) | val3 ; // # combine the 3 byte into one byte 
    //WiredSerial.print(data1[i], HEX);
    //WiredSerial.print(","); 
  } 
  //WiredSerial.println();

  
  digitalWrite(kPIN_LED, HIGH); // to test how long the data is send the the PC, to verifiy if it happens before a new DRDY itnerrupt

  for (int i = 3; i < 27; i++) {
    if (i < 26)
    {
      WiredSerial.print(data[i], HEX);
      WiredSerial.print(","); 
    }
    else
    {
      WiredSerial.println(data[i], HEX);
    }
  ...
  for (int i = 0; i < 9; i++) {
    ...
    {
      WiredSerial.println(data1[i], HEX);
    }
